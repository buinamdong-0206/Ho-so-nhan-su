...
async function startServer() {
  ...
  // API Proxy for external data
  app.get('/api/proxy/api/profiles/get/political-career', async (req, res) => {
    const profileId = req.query.profileId;
    try {
      const response = await fetch(`https://api.daihoidangtoanquoc.vn/api/profiles/get/political-career?profileId=${profileId}`);
  ...
  app.get('/api/proxy/api/profiles/get/items', async (req, res) => {
    try {
      const response = await fetch(`https://api.daihoidangtoanquoc.vn/api/profiles/get/items${req.url.split('items')[1]}`);
      const data = await response.json();
      res.json(data);
    } catch (error) {
  ...
  // Share route - returns compiled HTML
  app.get('/share/:listId', async (req, res) => {
    const listId = req.params.listId;
    
    try {
      // We'll use the REST API to fetch from Firestore to avoid complex SDK setup in server
